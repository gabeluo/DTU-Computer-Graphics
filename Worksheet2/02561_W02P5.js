...
window.onload = function init()
{
	...
	var mousepos = vec3(0.0, 0.0, 0.0);
	canvas.addEventListener("click", function (ev) {
		var bbox = ev.target.getBoundingClientRect();
		mousepos = vec3(2*(ev.clientX - bbox.left)/canvas.width - 1, 2*(canvas.height - ev.clientY + bbox.top - 1)/canvas.height - 1, 1-2*(index+1)/max_verts);
		...
		if (drawMode == 2 && circlePoints == 1) {
			if (index + numCirclePoints+1 < max_verts) {
				...
				for (var i = 0; i <= numCirclePoints; i++) {
					...
					vertices.push(vec3(radius * Math.cos(theta) + circleCentre[0], radius * Math.sin(theta) + circleCentre[1]));
